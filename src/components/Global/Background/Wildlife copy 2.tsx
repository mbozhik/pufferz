...
export default function Wildlife() {
  ...
  return (
    <div data-background="wildlife" className="absolute inset-0 overflow-hidden">
      ...
      {/* Static Seaweed */}
      {/* Left seaweed - 3 pieces */}
      {[0, 1, 2].map((index) => {
        const baseHeight = WILDLIFE_CONFIG.seaweed.size
        const responsiveHeight = isDesktop ? baseHeight : baseHeight * WILDLIFE_CONFIG.seaweed.mobileHeightMultiplier
        // Use predefined rotation values instead of Math.random() to avoid hydration errors
        const rotations = [-3, 2, -1] // Predefined small rotations for left seaweed
      ...
      {/* Right seaweed - 2 pieces */}
      {[0, 1].map((index) => {
        const baseHeight = WILDLIFE_CONFIG.seaweed.size
        const responsiveHeight = isDesktop ? baseHeight : baseHeight * WILDLIFE_CONFIG.seaweed.mobileHeightMultiplier
        // Use predefined rotation values instead of Math.random() to avoid hydration errors
        const rotations = [1, -2] // Predefined small rotations for right seaweed
